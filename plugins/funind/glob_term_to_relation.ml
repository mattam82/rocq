...
let raw_push_named (na, raw_value, raw_typ) env =
  ...
  | Name id -> (
    ...
    | None -> EConstr.push_named ProofVar (NamedDecl.LocalAssum (na, typ)) env
    | Some value -> EConstr.push_named ProofVar (NamedDecl.LocalDef (na, value, typ)) env
    )
      ...
      (Context.Rel.fold_outside
         (fun decl (env, ctxt) ->
           ...
           | LocalAssum (({binder_name = Name id} as na), t) ->
             ...
             (Environ.push_named ProofVar (LocalAssum (na, new_t)) env, mkVar id :: ctxt)
           | LocalDef (({binder_name = Name id} as na), v, t) ->
             ...
             observe
               ...
               ++ fnl () );
             let open Context.Named.Declaration in
             ( Environ.push_named ProofVar (LocalDef (na, new_v, new_t)) env
             , mkVar id :: ctxt ))
  ...
  let evd, env =
    Array.fold_right2
      (fun id (c, u) (evd, env) ->
        ...
        ( evd
        , EConstr.push_named ProofVar (LocalAssum (make_annot id ERelevance.relevant, t)) env
        ))
      funnames
    ...
    *)
    ...
    Util.Array.fold_left2
      (fun env rel_name rel_ar ->
        ...
        let r = ERelevance.relevant in
        (* TODO relevance *)
        EConstr.push_named ProofVar (LocalAssum (make_annot rel_name r, rex)) env)
