...
let ppprojrepr con = pp(Projection.Repr.print con)
...
let qprinter = UnivNames.quality_printer UnivNames.empty_binders
let sprinter = UnivNames.sort_printer UnivNames.empty_binders
let prlev l = UnivNames.pr_level_with_global_universes l
let prqvar q = UnivNames.pr_quality_with_global_universes (QVar q)
let prquality q = UnivNames.pr_quality_with_global_universes q
...
let ppqconstraints c = pp (ElimConstraints.pr qprinter c)
...
let ppaucontext auctx =
  ...
  let prqglobal q = prquality (QGlobal q) in
...
let constr_display csr =
  ...
  and sort_display = function
    ...
    | GSort (q, u) -> univ_display u; Printf.sprintf "GSort(%s, %i)" (Sorts.QGlobal.to_string q) !cnt
    | VSort (q, u) -> univ_display u; Printf.sprintf "VSort(%s, %i)" (Sorts.QVar.to_string q) !cnt
...
let print_pure_constr csr =
  ...
  and sort_display = function
    ...
    | GSort (q, u) -> open_hbox();
        print_string "GSort("; pp (str @@ QGlobal.to_string q); print_string ", "; pp (Universe.raw_pr u); print_string ")"; close_box()
    | VSort (q, u) -> open_hbox();
        print_string "VSort("; pp (QVar.raw_pr q); print_string ", "; pp (Universe.raw_pr u); print_string ")"; close_box()
