...
type raw_hint = {
  ...
  rhint_type : types;
  rhint_uctx : UnivGen.sort_context_set;
...
type hint = {
  ...
  hint_type : types;
  hint_uctx : UnivGen.sort_context_set;
...
let merge_context_set sigma ctx =
  Evd.merge_sort_context_set Evd.univ_flexible ~src:UState.Internal sigma ctx

let instantiate_hint env sigma p =
  let mk_clenv { rhint_term = c; rhint_type = cty; rhint_uctx = ctx; rhint_arty = ar } =
    let sigma = merge_context_set sigma ctx in
...
let connect_hint_clenv h gl =
  ...
  Clenv.clenv_refresh env sigma (Some ctx) clenv

let fresh_hint env sigma h =
  let { hint_term = c; hint_uctx = ctx } = h in
  (* Refresh the instance of the hint *)
  let (subst, ctx) = UnivGen.fresh_sort_context_instance h.hint_uctx in
