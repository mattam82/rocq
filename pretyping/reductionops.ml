...
let check_inductive_instances ~flex:_ ~nargs cv_pb variances u1 u2 univs =
  ...
  if check_cumul_contraints univs csts then Result.Ok univs else Result.Error None
...
let is_conv_nounivs ?(reds=TransparentState.full) env sigma t1 t2 =
  if EConstr.eq_constr_nounivs sigma t1 t2 then true
    ...
    try
      let env = Environ.set_universes (Evd.universes sigma) env in
      let ignore_univs = let open UCompare in {
        compare_sorts = (fun _ _ _ _ () -> Ok ());
        compare_instances = (fun ~flex:_ _ _ () -> Ok ());
        compare_cumul_instances = (fun ~flex:_ ~nargs:_ _ _ _ _ () -> Ok ());
      }
      in
...
let enforce_leq_sort s1 s2 ucsts =
  match s1, s2 with
  | Prop, (Set | Type _) -> Univ.UnivConstraints.empty
  | _ ->
  if Sorts.Quality.equal (Sorts.quality s1) (Sorts.quality s2) then
    UnivSubst.enforce_leq (Sorts.univ_of_sort s1) (Sorts.univ_of_sort s2) ucsts
  else
...
let infer_leq (univs, cstrs as cuniv) s s' =
  if UGraph.check_leq_sort Sorts.Quality.equal univs s s' then Result.Ok cuniv
    ...
    let ucstrs' = enforce_leq_sort s s' Univ.UnivConstraints.empty in
    Result.Ok (fst (UGraph.merge_constraints ucstrs' univs), UnivConstraints.union cstrs ucstrs')
  with UGraph.UniverseInconsistency err -> Result.Error (Some (Univ err))
...
let infer_inductive_instances ~flex ~nargs cv_pb variance u1 u2 (univs,csts) =
