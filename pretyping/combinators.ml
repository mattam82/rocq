...
let get_sigmatypes sigma ~sort ~predsort =
  ...
  let sigma, ty = Evd.fresh_global env sigma (lib_ref ("core."^which^".type")) in
  ...
  let make_proj sigma gr = Evd.fresh_global env sigma gr in
  let sigma, p1 = make_proj sigma (lib_ref ("core."^which^".proj1")) in
  let sigma, p2 = make_proj sigma (lib_ref ("core."^which^".proj2")) in
